...
function updateBotCount(a, b) {
  ...
  if (0 == c) {
    ...
    $("#botCount").html('<font color="#7FFF00">' + c + " / " + 50 + "</font>");
  }
}
function startLocalBots() {
  var a$$0 = 0;
  for (;a$$0 < FZZBOT.startBotAmount;a$$0++) {
    FZZBOT.localBotsAlive[a$$0] = false;
    FZZBOT.localBots[a$$0] = new Worker(URL.createObjectURL(new Blob(["(" + generateBotFunction() + ")()"], {
      type : "text/javascript"
    })));
    FZZBOT.localBots[a$$0].onmessage = function(a) {
      var b = a.data;
      switch(b.name) {
        case "add":
    ...
    FZZBOT.localBots[a$$0].postMessage({
      name : "botID",
      botID : a$$0
    });
  }
...
function startRemoteBots() {
  var a = 0;
  for (;a < 3;a++) {
    FZZBOT.remoteBots[a] = new Worker(URL.createObjectURL(new Blob(["(" + generateBotFunction() + ")()"], {
      type : "text/javascript"
    })));
  }
}
function sendLocalBotsMessage(a) {
  for (i in FZZBOT.localBots) {
    FZZBOT.localBots[i].postMessage(a);
  }
}
function sendRemoteBotsMessage(a) {
  for (i in FZZBOT.remoteBots) {
    FZZBOT.remoteBots[i].postMessage(a);
  }
}
function insertCore() {
  ...
  f.onload = function() {
    ...
    script = replaceNormalFile(script, "if(h.MC&&h.MC.onPlayerSpawn)", "FZZBOT.playerSpawned();if(h.MC&&h.MC.onPlayerSpawn)");
    script = replaceNormalFile(script, "if(h.MC&&h.MC.onPlayerDeath)", "FZZBOT.playerDied();if(h.MC&&h.MC.onPlayerDeath)");
    script = replaceNormalFile(script, "if(h.MC&&h.MC.onAgarioCoreLoaded)", "FZZBOT.onAgarioCoreLoaded();if(h.MC&&h.MC.onAgarioCoreLoaded)");
    script = replaceNormalFile(script, "if(h.MC&&h.MC.onDisconnect)", "FZZBOT.playerDisconnected();if(h.MC&&h.MC.onDisconnect)");
    script = replaceNormalFile(script, "connect:function(a){", "connect:function(a){FZZBOT.playerConnected(a);");
    script = replaceNormalFile(script, "sendSpectate:function(){", "sendSpectate:function(){FZZBOT.playerSpectated();");
    script = replaceNormalFile(script, "sendNick:function(a){", "sendNick:function(a){FZZBOT.updateNickname(a);");
    script = replaceNormalFile(script, "setTarget:function(a,b){", "setTarget:function(a,b){if(FZZBOT.stopMovement){a = $('#canvas').width() / 2; b = $('#canvas').height() / 2;}");
    script = replaceRegexFile(script, /(\w\[\w\+(\d+)>>3]=(\w);\w\[\w\+(\d+)>>3]=(\w);\w\[\w\+(\d+)>>3]=(\w);\w\[\w\+(\d+)>>3]=(\w);)/i, "$1 if(FZZBOT.setMapCoords){FZZBOT.setMapCoords($3,$5,$7,$9,$2,$8);}");
    script = replaceRegexFile(script, /([\w$]+\(\d+,\w\[\w>>2\]\|0,(\+\w),(\+\w)\)\|0;[\w$]+\(\d+,\w\[\w>>2\]\|0,\+-(\+\w\[\w\+\d+>>3\]),\+-(\+\w\[\w\+\d+>>3\])\)\|0;)/i, "$1 FZZBOT.playerX=$4; FZZBOT.playerY=$5;");
    script = replaceRegexFile(script, /if\((\+\w\[\w>>3\])<1\.0\){/i, "if($1 < FZZBOT.zoomResetValue){");
    ...
    script = replaceRegexFile(script, /(\w)(=\+\w\[\w>>3\]\*\+\w\()(.\d)(,\+\w\);)/i, "$1$2 (FZZBOT.zoomSpeedValue||0.9) $4 FZZBOT.zoomValue=$1;");
    script = replaceRegexFile(script, /(\w=\w\[\w>>2\]\|0;)((\w\[\w>>3\])=(\w);)(\w\[\w>>0\]=a\[\w>>0\];)/i, "$1 if(!FZZBOT.autoZoom){$3 = FZZBOT.zoomValue;}else{$2}$5");
    script = replaceRegexFile(script, /((\w)=(\+\(\(\w\[\w\+\d+>>\d.*;)(\w)=(\+\(\(\w\[.*\/2\|\d\)\|0\)\/\w\+\s\+\w\[\w\+\d+>>3\];).*\4=\4<\w\?\w:\w;)/, "FZZBOT.mouseX = $3 FZZBOT.mouseY = $5 $1");
    eval(script);
  };
  f.send();
}
...
function drawMinimap() {
  if (null != miniMap ? minimapCtx.clearRect(0, 0, 200, 200) : (miniMap = document.getElementById("minimap"), minimapCtx = miniMap.getContext("2d"), miniMap.width = 400, miniMap.height = 400, miniMap.style.width = "200px", miniMap.style.height = "200px", minimapCtx.scale(2, 2)), FZZBOT.mapOffsetFixed && FZZBOT.drawMinimap) {
    minimapCtx.globalAlpha = 0.4;
    minimapCtx.fillStyle = "#000000";
    minimapCtx.fillRect(0, 0, miniMap.width, miniMap.height);
    var a = 200 / FZZBOT.mapSize;
    var b = 200 / FZZBOT.mapSize;
...
function connectToFZZBOTServer() {
  socket = io.connect("ws://127.0.0.1:8003", {
    reconnection : true,
...
socket.on("force-uuid", function(data) {
  ...
  socket.on("command", function(a) {
    ...
    switch(a.name) {
      ...
      case "position":
        ...
        break;
      case "count":
        FZZBOT.serverBots = a.count;
        break;
      case "auth":
        FZZBOT.isAuthorized = true;
  ...
  socket.on("bots", function(a) {
    if ("server" == a.name) {
      FZZBOT.remoteBotsServer = a.server;
    }
...
function updateBotNames() {
  sendLocalBotsMessage({
    name : "names",
    botNames : FZZBOT.botNames
  });
  if (FZZBOT.isAuthorized) {
    sendCommand({
      name : "names",
      botNames : FZZBOT.botNames
    });
...
function emitLocalPosition() {
  var a = FZZBOT.mouseX;
  var b = FZZBOT.mouseY;
  if (!FZZBOT.moveToMouse) {
    a = FZZBOT.playerX;
    b = FZZBOT.playerY;
  }
  sendLocalBotsMessage({
    name : "position",
    x : a + FZZBOT.mapOffsetX,
    y : b + FZZBOT.mapOffsetY
  });
}
function emitPosition() {
  ...
  sendCommand({
    name : "position",
    x : FZZBOT.realPlayerX,
    y : FZZBOT.realPlayerY,
    botX : a + FZZBOT.mapOffsetX,
    botY : b + FZZBOT.mapOffsetY
  });
}
function transmit_current_server(a) {
  if (a || last_transmited_game_server != FZZBOT.server) {
    last_transmited_game_server = FZZBOT.server;
    sendCommand({
      name : "servers",
  ...
  return function() {
    ...
    function getBotCore() {
      ...
      e.onload = function() {
        ...
        script = replaceRegexFile(script, /(\w)=\+\(\(\w\[\w\+\d+>>\d.*;(\w)=\+\(\(\w\[.*\/2\|\d\)\|0\)\/\w\+\s\+\w\[\w\+\d+>>3\];/, "$1 = FZZBOT.newX; $2 = FZZBOT.newY;");
        script = replaceNormalFile(script, "if(h.MC&&h.MC.onPlayerSpawn)", "FZZBOT.playerSpawned();if(h.MC&&h.MC.onPlayerSpawn)");
        script = replaceNormalFile(script, "if(h.MC&&h.MC.onPlayerDeath)", "FZZBOT.playerDied();if(h.MC&&h.MC.onPlayerDeath)");
        script = replaceNormalFile(script, "if(h.MC&&h.MC.onAgarioCoreLoaded)", "FZZBOT.onAgarioCoreLoaded();if(h.MC&&h.MC.onAgarioCoreLoaded)");
        script = replaceNormalFile(script, "if(h.MC&&h.MC.onDisconnect)", "FZZBOT.playerDisconnected();if(h.MC&&h.MC.onDisconnect)");
        script = replaceNormalFile(script, "h.MC&&h.MC.corePendingReload", "FZZBOT.reloadCore();h.MC&&h.MC.corePendingReload");
        script = replaceRegexFile(script, /(\w\[\w\+(\d+)>>3]=(\w);\w\[\w\+(\d+)>>3]=(\w);\w\[\w\+(\d+)>>3]=(\w);\w\[\w\+(\d+)>>3]=(\w);)/i, "$1 if(FZZBOT.setMapCoords){FZZBOT.setMapCoords($3,$5,$7,$9,$2,$8);}");
        script = replaceRegexFile(script, /([\w$]+\(\d+,\w\[\w>>2\]\|0,(\+\w),(\+\w)\)\|0;[\w$]+\(\d+,\w\[\w>>2\]\|0,\+-(\+\w\[\w\+\d+>>3\]),\+-(\+\w\[\w\+\d+>>3\])\)\|0;)/i, "$1 FZZBOT.playerX=$4; FZZBOT.playerY=$5; FZZBOT.setPath();");
        script = replaceRegexFile(script, /(do\sif\(\w\){)((\w)=!\(\+\w\[\w>>2]<=20.0\);)(.+,\w\[\w>>2\]\|0,(\+\(\+\w\[\w>>2\]\)),(\+\(\+\w\[\w>>2\]\)),\+\((\+\w\[\w>>2\]))/, "$1var cellSize=$7;$2if(!$3){FZZBOT.recordPellet($5,$6,cellSize);}$4");
        eval(script);
      };
      e.send(null);
    }
    ...
    self.FZZBOT = {
      ...
      playerDisconnected : function() {
        ...
        if (self.core) {
          if (null != FZZBOT.server) {
            core.connect(FZZBOT.server);
          }
        }
      },
      reloadCore : function() {
    ...
    onmessage = function(a) {
      ...
      switch(b.name) {
        case "botID":
          FZZBOT.botID = b.botID;
          break;
        case "server":
          FZZBOT.server = b.server;
          if (self.core) {
            if (null != b.server) {
          ...
          break;
        case "position":
          if (FZZBOT.collectPellets && (99999 != FZZBOT.pelletTargetX && 99999 != FZZBOT.pelletTargetY)) {
            FZZBOT.newX = FZZBOT.pelletTargetX;
            FZZBOT.newY = FZZBOT.pelletTargetY;
          } else {
            FZZBOT.newX = b.x - FZZBOT.mapOffsetX;
            FZZBOT.newY = b.y - FZZBOT.mapOffsetY;
          }
          break;
        case "split":
          if (self.core) {
        ...
        case "names":
          if (null == b.botNames) {
            FZZBOT.botName = "FZ BOT";
            break;
          }
          FZZBOT.botName = b.botNames[getRandomInt(0, b.botNames.length - 1)];
          break;
        case "disconnect":
          FZZBOT.server = null;
          if (self.core) {
          ...
          break;
        case "collectPellets":
          FZZBOT.collectPellets = b.collectPellets;
          break;
        default:
    ...
    setInterval(function() {
      FZZBOT.realPlayerX = FZZBOT.mapOffsetX + FZZBOT.playerX;
      FZZBOT.realPlayerY = FZZBOT.mapOffsetY + FZZBOT.playerY;
      postMessage({
        botID : FZZBOT.botID,
        name : "position",
        x : FZZBOT.realPlayerX,
...
var client_uuid = escapeHtml(localStorage.getItem("FZZBOT_uuid"));
if (null === client_uuid || 15 != client_uuid.length) {
  ...
  localStorage.setItem("FZZBOT_uuid", client_uuid);
}
window.FZZBOT = {
  server : true,
  playerName : "",
  ...
  playerSpawned : function() {
    FZZBOT.isAlive = true;
    changeNicknameOnBall("player_pointer", FZZBOT.playerName);
  ...
  loadCore : function() {
    ...
    if (null !== b$$0) {
      FZZBOT.botNames = validateNames(b$$0);
      if (null !== FZZBOT.botNames) {
        $("#botnames").val(b$$0);
      }
      updateBotNames();
    }
    ...
    $(document).keydown(function(a) {
      switch(a.which) {
        case 65:
          FZZBOT.moveToMouse = !FZZBOT.moveToMouse;
          if (FZZBOT.moveToMouse) {
          ...
          break;
        case 68:
          FZZBOT.stopMovement = !FZZBOT.stopMovement;
          if (FZZBOT.stopMovement) {
        ...
        case 82:
          ...
          break;
        case 77:
          FZZBOT.drawMinimap = !FZZBOT.drawMinimap;
          if (FZZBOT.drawMinimap) {
            $("#botcanvas").show();
          } else {
            $("#botcanvas").hide();
          }
    ...
    addBallToMinimap(true, "player_pointer", FZZBOT.playerName, FZZBOT.realPlayerX, FZZBOT.realPlayerY, "#00FF00", false);
    addBallToMinimap(true, "player_death", "Last Death", FZZBOT.realPlayerX, FZZBOT.realPlayerY, "#FF2400", false);
    addBallToMinimap(true, "player_spectate", "Spectate", FZZBOT.realPlayerX, FZZBOT.realPlayerY, "#0000FF", false);
    connectToFZZBOTServer();
    insertCore();
    setInterval(function() {
      MC.FZZBOTFreeCoins();
    }, 5E3);
...
CanvasRenderingContext2D.prototype._fillText = CanvasRenderingContext2D.prototype.fillText, CanvasRenderingContext2D.prototype.fillText = function() {
  ...
  if ("Leaderboard" === arguments[0]) {
    if ("" != tempLeaderBoard) {
      FZZBOT.leaderboardData = tempLeaderBoard;
      $("#leaderboard").val(FZZBOT.leaderboardData);
    }
...
minimapBalls = {}, MinimapBall.prototype = {
  draw : function(a, b, c) {
    if (this.visible) {
      ...
      var d = ((this.isDefault ? this.x : this.lastX) + FZZBOT.mapOffset) * b;
      var e = ((this.isDefault ? this.y : this.lastY) + FZZBOT.mapOffset) * c;
...
b.open("GET", "/mc/agario.js", true), b.onload = function() {
  ...
  script = replaceRegexFile(script, /var\si2=window.document.createElement..script..+head.appendChild.i2../i, "FZZBOT.reloadCore();");
  script = replaceRegexFile(script, /(showFreeCoins:function\(\)\{var.*showContainer\(\);if\(([a-zA-Z0-9]+[a-zA-Z0-9]+.user.userInfo==null).*false\);([a-zA-Z0-9]+[a-zA-Z0-9]+.triggerFreeCoins\(\)).*this.onShopClose\)\)\}},)/, "$1 FZZBOTFreeCoins: function(){if($2){return;}$3;},");
  ...
  FZZBOT.realPlayerX = FZZBOT.mapOffsetX + FZZBOT.playerX;
  FZZBOT.realPlayerY = FZZBOT.mapOffsetY + FZZBOT.playerY;
  moveBallOnMinimap("player_pointer", FZZBOT.realPlayerX, FZZBOT.realPlayerY);
  moveBallOnMinimap("player_spectate", FZZBOT.realPlayerX, FZZBOT.realPlayerY);
}, 50);
