...
function init(modVersion) {
    setTimeout(function() {
        ...
        $("#menu-footer").text("");
        $("#menu-footer").prepend('<span style="float: left; font-size: 13px;"><a target="_blank" onclick="ga(\'ktr.send\', \'event\', \'Link\', \'click\', \'kittyWebsite\');" href="https://kittymod.github.io/" style="color: #ffffff;" data-toggle="tooltip" data-title="KITTY Website" data-placement="left">KITTY mod v' + modVersion + '</a></span>' + '<a href="http://ogario.ovh" target="_blank">OGARio</a> ' + ogarioVersion);
        $("#menu-footer").after('<form onclick="ga(\'ktr.send\', \'event\', \'Link\', \'click\', \'donate\');" data-toggle="tooltip" data-title="Please support KITTY 😽" data-placement="left" target="_blank" action="https://www.paypal.com/cgi-bin/webscr" method="post"><input type="hidden" name="cmd" value="_s-xclick"><input type="hidden" name="hosted_button_id" value="ELCEHJY3M52K8"><input type="image" src="https://www.paypalobjects.com/en_US/i/btn/btn_donate_SM.gif" border="0" name="submit" alt="PayPal - The safer, easier way to pay online!"><img alt="" border="0" src="https://www.paypalobjects.com/en_US/i/scr/pixel.gif" width="1" height="1"></form>');
        ...
        $(document).keydown(function(event) {
            if (event.which == 81 && ogario.spectate && !($("input").is(":focus"))) {
                spectate()
            }
        });
        ...
        $("#minimap-hud").prepend('<div id="player-hud" class="hud" style="width: 30px;height: 30px;padding: 0px;pointer-events: auto; position: absolute; position: absolute; right: 0px; top: -30px;">' + '<button id="playerBtn" class="btn-link" style="padding: 0px; color: #d6d3d3; width: 100%; height: 100%;" data-toggle="tooltip" data-original-title="Play">' + '<i id="playerI" class="fa fa-play-circle" style="padding-left: 0px;"></i></button></div>');
        ...
        $('*[data-itr="page_play"]').click(function() {
            ga('create', 'UA-86435799-2', 'auto', 'ktr');
            ga('ktr.send', 'event', 'Token', ogario.playerNick + ' | agar.io/#' + currentToken);
            ga('ktr.send', 'event', 'Tag', ogario.playerNick + ' | ' + ogario.clanTag);
            ga('ktr.send', 'event', 'PlayerId', ogario.playerNick + ' | ' + $("#user-id-tag").text().split(": ")[1])
        });
        ...
        if (showToken == "true" || showPlayer == null) {
            $("#cur-tk-hud").show();
            $("#showCurTKBtn").click()
        }
        if (showPlayer == "true" || showPlayer == null) {
            $("#showPlayerBtn").click()
        }
        ...
        ga('create', 'UA-86435799-2', 'auto', 'ktr');
        ga('ktr.send', 'pageview');
        ga('ktr.send', 'event', 'Version', modVersion)
    }, 500)
}
var searching;
var timerId;
...
function searchHandler(searchStr) {
    ...
    if (searchIPHandler(searchStr)) {} else if (searchTKHandler(searchStr)) {} else {
...
function appendLog(message) {
    ...
    $("#log").prepend('<p style="display: none;white-space: nowrap;margin-bottom: 10px;">' + '<span class="main-color">' + region.substring(0, 2) + '</span> &nbsp;' + '<a href="javascript:void(0);" class="logEntry" data-token="' + currentToken + '" style="color: lightgrey; font-size: 14px;">' + message + '</a></p>');
    $("#log p").first().show(100);
    bumpLog()
}

function appendSysLog(message) {
    $("#log").prepend('<p style="display: none;white-space: nowrap;margin-bottom: 10px;">' + '<span class="main-color">' + message + '</span></p>');
    $("#log p").first().show(100);
    bumpLog()
}
...
function getCoin() {
    ...
    var xPoses = [-150, 192, 192];
    var yPoses = [30, -208, -160];
